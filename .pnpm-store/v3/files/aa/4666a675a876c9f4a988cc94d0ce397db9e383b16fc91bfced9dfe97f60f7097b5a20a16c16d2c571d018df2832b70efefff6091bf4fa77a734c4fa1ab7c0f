...
function getConfigResolver() {
    ...
    try {
        ...
        return (configResolver = (filePath) => {
            const absolutePath = path_1.resolve(process.cwd(), filePath);
...
function isValidFilename(filename) {
    ...
    if (fs_1.existsSync(dir) && fs_1.statSync(dir).isDirectory()) {
        if (fs_1.existsSync(filename) && fs_1.statSync(filename).isDirectory()) {
            return false;
        }
        return true;
    }
